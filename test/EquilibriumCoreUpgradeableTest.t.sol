...
contract EquilibriumCoreUpgradeableTest is Test {
    ...
    EquilibriumCoreUpgradeable public core_upgradeable;
    EquilibriumCoreUpgradeable public core;
    ...
    function setUp() public {
        ...
        // Proxy Setup
        core_upgradeable = new EquilibriumCoreUpgradeable();

        ERC1967Proxy proxy = new ERC1967Proxy(address(core_upgradeable), ""); 

        core = EquilibriumCoreUpgradeable(address(proxy));
        
        // initialize imediately after deploying
        core.initialize(
            address(weth_mock), address(wbtc_mock), address(weth_feed), address(wbtc_feed)
        );
        // End of Proxy Setup
    ...
    *.*.*.*.*.*.*.*.*.**.*.*.*.*.*.*.*.*.*/
    ...
    function testVersionNumber() external {
        uint256 version_number = core.get_version();
        assertEq(version_number, uint256(1));
    }
}
