...
contract InstanceEquilibriumCore is EquilibriumCore {
    ...
    constructor() EquilibriumCore(WETH, WBTC, WETH_PRICE_FEED, WBTC_PRICE_FEED) {}
    ...
    function calculateEquilibriumAmountToMint(uint256 _collateralAmountAddedInUsd) external pure returns (uint256 quilibrium_amount) {
        quilibrium_amount = super._calculateEquilibriumAmountToMint(_collateralAmountAddedInUsd);
    }

    function getUsdValue(address _collateralAddress, uint256 _amountOfCollateral) external view returns (uint256 usd_value) {
