...
contract DSCEngine is ReentrancyGuard {
    ...
    // @audit-info using unchcked increament decreament of bounded asset/balance.
     ...
     * #ADD It's imporrtant to first increase collateral amount and then mint DSC.?
     */
     ...
     * #add First off we should burn the user's DSC tokens if we want to check the revertHealthfactor during this process.
     */
        ...
        isAllowedToken(tokenCollateralAddress)
    {
        ...
        // @bug using safeTransfer function belongs to safeIERC20 library to avoid arbitary token addresses.
    ...
    function _calculateHealthFactor(uint256 totalDscMinted, uint256 collateralValueInUsd)
        ...
        returns (uint256)
    {
        if (totalDscMinted == 0) return type(uint256).max; //q why?
...
// import {Console2} from "forge-test/Console2.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {AggregatorV3Interface} from "@chainlink/contracts/src/v0.8/interfaces/AggregatorV3Interface.sol";
...
contract EquilibriumEngine {
    ...
    */
    ...
    function getUsdValue(address _tokenAddress, uint256 _amount) external view returns (uint256) {
        AggregatorV3Interface priceFeed = AggregatorV3Interface(_tokenAddress); // should be the corresponding priceFeed address instead
